...
it('renders workspace stage controls and counts matching search results', function (): void {
    ...
    $component = livewire(BoardKanban::class, ['record' => $board->slug])
        ...
        ->assertSee('delay: this.touchDragDelay', escape: false)
        ->assertSee('delayOnTouchOnly: true', escape: false)
        ->assertSee('touchStartThreshold: this.touchStartThreshold', escape: false)
        ->assertSee('x-init="$nextTick(() => configureCardTouchDragging($el))"', escape: false)
        ->assertSee("sortable.option('delay', this.touchDragDelay)", escape: false)
        ->assertSee("sortable.option('delayOnTouchOnly', true)", escape: false)
