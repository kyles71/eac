...
it('reconciles payment attempts every five minutes', function (): void {
    $event = collect(Schedule::events())
        ->first(fn (Event $event): bool => str_contains($event->command ?? '', 'installments:reconcile-payment-attempts'));
